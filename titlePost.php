...
<div class="well">
    <form action="titlePost.php" method="POST">
    请输入要查询的帖子名:&nbsp;<input type="text" name="post_title"/>
...
if (isset($_POST['post_title'])) {
    ...
    $post_title = sanitizeString($_POST['post_title']);
    $query_sql = "select post_id, post_title,post_content,post_time,post_writer from writepost where post_title=?";
    $result = $sqlconn -> prepare($query_sql);
    $result -> bind_param("s", $post_title);
    $result->bind_result($post_id,$post_title,$post_content,$post_time,$post_writer); //绑定结果
    $result -> execute();
    //绑定结果
