...
<div class="btn-toolbar">
    ...
    <button class="btn"href="idUser.php">按学号查询</button>
</div>
<div class="well">
	<form action="idUser.php" method="POST">
	请输入要查询的学号:&nbsp;<input type="text" name="account"/>
...
if (isset($_POST['account'])) {
	...
	$query_sql = "select sno,nickname,faculty,remark,email,status from student where sno=?";
	$result = $sqlconn -> prepare($query_sql);
	$result -> bind_param("s", $account);
	$result->bind_result($account,$nickname,$faculty,$remark,$email,$status); //绑定结果
	$result -> execute();
	//绑定结果
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-footer">
        <button class="btn" data-dismiss="modal" aria-hidden="true">按钮</button>
        <button class="btn btn-danger" data-dismiss="modal">按钮</button>
    </div>
</div>
