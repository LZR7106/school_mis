...
$query_sql = "select user_id,user_name,user_when,user_where,user_department,user_main_sales,is_agent,user_state from tbl_user ORDER BY user_id asc limit $baserow,$offset";
$result = $sqlconn -> prepare($query_sql);//SQL预处理
$result -> execute();//执行
$result -> bind_result($user_id, $user_name, $user_when, $user_where, $user_department, $user_main_sales, $is_agent, $user_state);//绑定结果
...
while ($result -> fetch()) {
    ...
    <tr>
		...
		<td align="center" valign="middle"  >$is_agent </td>
		<td align="center" valign="middle"  >$zt </td>
		 <td>
               ...
               <a href="disableBusiness.php?user_state=$user_state&user_id=$user_id"><i class="icon-user"></i></a>
          	   ...
          	   <a href="productList.php?user_id=$user_id"><i class="icon-star"></i></a>
